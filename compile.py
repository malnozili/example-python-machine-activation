...
class FrappeAppCompiler:
    ...
    def copy_all_needed_fiels(self):
        """
        Copy all files to build folder
        :return:
        """
        print("#################### Copy all files modules ####################")
        files = os.listdir('.')

        print(dir(files))

    def copy_comile_fiels_to_orginal_dir(self) -> None:
        modules = self.ext_modules()
        for _ in modules:
            base_folder_name = _.name.replace('.', '/').split('/')
            module_folder = '/'.join(base_folder_name[:-1])
            if base_folder_name[0] == 'build' or module_folder == '':
                continue
            full_build_folder = f'{self.build_directory}/{module_folder}/'
            files = os.listdir(full_build_folder)
            for fname in files:
                # copying the files to the
                # destination directory
                try:
                    shutil.copy2(os.path.join(full_build_folder, fname), module_folder)
                except IsADirectoryError:
                    continue
                dis_python_files = os.listdir(module_folder)
                for i in dis_python_files:
                    py_ext = i.split('.')
                    if py_ext[-1] == 'py':
                        if i != '__init__.py':
                            os.remove(f'{module_folder}/{i}')


    def rm_all_python_files_on_build_folder(self) -> None:
        """
        This function removes all python files
        :return:
        """
        for dirpath, dirs, files in os.walk('build'):
            for filename in files:
                ...
                if fname.endswith('.py'):
                    files_ignore = ['__init__.py', 'setup.py']
                    if filename not in files_ignore:
                        os.remove(fname)
    ...
    def compile_project(self) -> None:
        ...
        self.rm_all_python_files_on_build_folder()
        # self.copy_comile_fiels_to_orginal_dir()

if __name__ == "__main__":
    compiler = FrappeAppCompiler(ignored_dirs=['.venv', 'build'], ignored_files=['compile.py'],
                                 other_dirs_needed=['company_wallet'],
                                 other_files_needed=['setup.py',
                                                     'requirements.txt'])
    compiler.set_ignored(ignored_dirs=['.venv'])
